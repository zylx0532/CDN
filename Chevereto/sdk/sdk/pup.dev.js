...
(function() {
	var PUP = {
		...
		vendors: {
				...
				editorValue: function(str) {
					...
					if(typeof str == 'string') {
						if(this.useCustomEditor()) {
							var insert = '<p>' + str + '</p>';
							this.getEditorFn().insert.html(this.editorValue() !== '' ? (nl + insert) : insert);
						} else {
			...
			XF2: {
				...
				editorValue: function(str, id) {
					...
					var type = edFn.ed.bbCode.isBbCodeView() ? ['bbCode', 'getBbCode', 'insertBbCode'] : ['html', 'get', 'insert'];
					...
					if(typeof str == 'string') {
						...
						if(type[0] == 'html') {
							...
							var XHR = XF.ajax('POST', XF.canonicalizeUrl('index.php?editor/to-bb-code'), {
							  html: str
							});
				...
				useCustomEditor: function() {
					return typeof XF.getEditorInContainer($('.js-editor')) !== typeof undefined;
				}
			}
		},
		...
		insertTrigger: function() {
			...
			for (var i=0; i<areas.length; i++) {
				...
				target.insertAdjacentHTML({before: 'beforebegin', after: 'afterend'}[this.settings.siblingPos],
	...
	for(var i=0; i<redactor2.length; i++) {
		PUP.vendors[redactor2[i]] = Object.assign(Object.assign({}, PUP.vendors.redactor2), PUP.vendors[redactor2[i]]);
	}
	PUP.prepare();
})();